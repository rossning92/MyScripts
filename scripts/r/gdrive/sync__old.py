...
def install_gdrive():
    ...
    if not exists("gdrive.exe"):
        ...
        download(
            "https://github.com/gdrive-org/gdrive/releases/download/2.1.0/gdrive-windows-x64.exe",
            filename="gdrive.exe",
        )
...
def get_file_id(name):
    s = (
        Popen(
            "gdrive list --name-width 0 -q \"trashed = false and 'root' in parents\"",
...
def create_dir(name):
    ...
    if fileId is None:
        s = (
            Popen('gdrive mkdir "%s"' % name, stdout=subprocess.PIPE)
            .stdout.read()
            .decode("utf-8")
        )
        print(s)
