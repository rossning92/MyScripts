...
async function openFile(context, filePath) {
    if (fs.existsSync(filePath)) {
        if (new RegExp(".(md|" + SOURCE_FILE_EXT + ")$", "g").test(filePath)) {
            ...
            const selection = await vscode.window.showQuickPick(["Open", "Trim Video"], {
...
function readResourceFile(context, fileName) {
...
function createVideoEditPanel() {
    ...
    if (!videoEditPanel) {
        videoEditPanel = vscode.window.createWebviewPanel("videoEdit.preview", "Video Edit", {
...
async function openFileUnderCursor(context) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
...
function insertLineOrReplaceSelection(text) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
...
function insertOrUpdateRecording(editor, newPath) {
    const selection = editor.selection;
    if (selection.isEmpty) {
        ...
        if (match) {
            ...
            const range = new vscode.Range(lineNumber, start, lineNumber, start + match[0].length);
            editor.edit((editBuilder) => {
...
function openTerminal({ name, args, singleInstance = true, cwd, }) {
    if (singleInstance) {
    ...
    terminal.show(true // the terminal will not take focus.
    );
}
function startMovyServer(context, file) {
    // Return random port number between 10000 and 20000
    let port = Math.floor(Math.random() * 10000) + 10000;
    const args = [
        "run_script",
...
function exportVideo({ extraArgs, exportSelectedText = true, } = {}) {
    const activeDir = getActiveDir();
    if (!activeDir) {
        return;
    }
...
function registerCreatePowerpointCommand(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("videoEdit.createPowerpointOverlay", async () => {
        ...
        insertLineOrReplaceSelection(`{{ overlay('${filePath}', n=1, pos=(960, 540), t='as') }}`);
    }));
}
...
async function createNewDocument({ dir, func, fileNamePlaceHolder = "a space separated name", initContent = "", extension = "", extraParams = "", }) {
    ...
    insertLineOrReplaceSelection(`{{ ${func}('${dir}/${name}${extension}'${extraParams}) }}`);
    const document = await vscode.workspace.openTextDocument(filePath);
    await vscode.window.showTextDocument(document);
    return filePath;
}
