...
for k, v in data.items():
    if k.upper() == "PATH":
        s = check_output(r"reg query HKCU\Environment /v PATH").decode()
        s = re.search(r"PATH\s+REG_SZ\s+(.*)", s).group(1).strip()
        paths = s.split(";")
        paths = [x for x in paths if x.strip() != "" and x not in data["PATH"]]
        s = ";".join(paths)
        print(f"PATH={s}")
        call(rf'reg add HKCU\Environment /v {k} /d "{s}" /f')
    else:
        print(f"Delete ENV: {k}")
        call(rf'reg delete "HKCU\Environment" /v {k} /f', check_call=False)
