...
Once the task is completed, you must reply with the full result starting with "TASK COMPLETED: ".
"""
        if code_execution
        else ""
    )
...
Once the task is completed, you must reply with a message that starts with "TASK COMPLETED: " and output full result.
...
class PyAgentMenu(ChatMenu):
    ...
    def __update_prompt(self):
        prompt = " agent : " + _get_agent_name(self.__agent_file) + "\n"
        prompt += f"&tools : {self.__agent['tools']}\n"
        prompt += f"  &ctx : {self.__agent['context']}\n"
        prompt += "-------\n"
                ...
                for block in blocks:
                    with io.StringIO() as buf, redirect_stdout(buf):
                        ...
                        if return_val is not None or stdout:
                            ...
                            response_message = "The Python code finishes successfully."

        # Check if the task is completed
        result = re.findall(r"TASK COMPLETED:\s*([\S\s]+)", content, flags=re.MULTILINE)
        if len(result) > 0:
...
def _main():
    ...
    menu = PyAgentMenu(yes_always=True)
    menu.exec()
