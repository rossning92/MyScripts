...
After finishing the task, send a message beginning with "TASK COMPLETED:" and include the full result.
...
class PyAgentMenu(ChatMenu):
    ...
    def __update_prompt(self):
        ...
        prompt = f"""\
agent (^l): {_get_agent_name(self.__agent_file)}
 task (!p): {task.splitlines()[0] if task else ''}
tools (!t): {self.__agent['tools']}
  ctx (!c): {self.__agent['context']}
-------
"""
    ...
    def __check_code_blocks(self):
        ...
        match = re.findall(r"TASK COMPLETED:?\s*([\S\s]*)", content, flags=re.MULTILINE)
        if len(match) > 0:
            result = match[0]
