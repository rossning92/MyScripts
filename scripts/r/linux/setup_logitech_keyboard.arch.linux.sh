...
sudo bash -c 'cat >/usr/local/bin/disable_logitech_fn_keys.sh' <<EOF
#!/usr/bin/bash
...
sudo bash -c 'cat >/etc/udev/rules.d/42-logitech-keyboard.rules' <<EOF
SUBSYSTEM=="bluetooth", ACTION=="add", KERNEL=="hci0:*", RUN+="/usr/local/bin/disable_logitech_fn_keys.sh"
EOF

sudo bash -c 'cat >/etc/udev/rules.d/99-hidraw.rules' <<EOF
