...
@common.api
def screencap(
    file, args=None, title=None, size=(1920, 1080), callback=None, **kwargs
):
    if not os.path.exists(file) or common.force:
        record_app(
            args=args, file=file, size=size, callback=callback, title=title
        )
    return file
...
common.api(record_term, skip=True)
