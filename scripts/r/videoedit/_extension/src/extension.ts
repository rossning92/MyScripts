...
function setupDecorations(context: vscode.ExtensionContext) {
  ...
  function updateDecorations() {
    ...
    highlightText(
      /\b(anim|clip|codef|hl|md|comment|overlay|slide|video_end|cmd|bash|ipython|node)(?=\()/g,
      "#0000ff"
    );
...
async function createNewDocument({
  dir,
  func,
  fileNamePlaceHolder = "a space separated name",
  initContent = "",
  extension = "",
  extraParams = "",
}: {
...
function autoUpdateScreenRecordDir(context: vscode.ExtensionContext) {
